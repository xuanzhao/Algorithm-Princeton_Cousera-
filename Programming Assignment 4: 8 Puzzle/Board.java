...
public class Board {
    ...
    public Board(int[][] blocks) {
        ...
        this.manhattan = -1;
        this.blankSquare = new CoordinatePair();
        
        for (char i = 0; i < this.N; i++) {
            for (char j = 0; j < this.N; j++) {
                this.board[i][j] = (char)blocks[i][j];
                if (blocks[i][j] == 0) {
                    this.blankSquare.i = i;
                    this.blankSquare.j = j;
                }
            }
        }
    }
    ...
    public int manhattan() {
        ...
        for (char i = 0; i < this.N; i ++) {
            for (char j = 0; j < this.N; j++) {
                ...
                char boardValue = this.board[i][j];
        ...
        this.manhattan = distance; // cache distance
        return distance;
    }
    ...
    public Board twin() {
        ...
        if (blocks[0][0] != 0 && blocks[0][1] != 0) {
            ...
            blocks[0][0] = this.board[0][1];
            blocks[0][1] = this.board[0][0];
        } else {
            // otherwise, switch first two blocks on second row
            blocks[1][0] = this.board[1][1];
            blocks[1][1] = this.board[1][0];
        }
    ...
    public Iterable<Board> neighbors() {
        ...
        if (blankSquare.j > 0) {
            ...
            int[][] blocks = copyBoard();
            blocks[blankSquare.i][blankSquare.j] = this.board[blankSquare.i][blankSquare.j-1];
            blocks[blankSquare.i][blankSquare.j-1] = this.board[blankSquare.i][blankSquare.j];
            stack.push(new Board(blocks));
        }
        // blank square not right edge
        if (blankSquare.j < this.N-1) {
        ...
        if (blankSquare.i > 0) {
            ...
            int[][] blocks = copyBoard();
            blocks[blankSquare.i][blankSquare.j] = this.board[blankSquare.i-1][blankSquare.j];
            blocks[blankSquare.i-1][blankSquare.j] = this.board[blankSquare.i][blankSquare.j];
            stack.push(new Board(blocks));
        }
